    ...
    public void appendBody(List<ITextComponent> tooltip, IDataAccessor accessor, IPluginConfig config) {
        ...
        if(tile.hasEntity() && tile.hasEnvironment()){
            tooltip.add(new StringTextComponent(
                    new TranslationTextComponent("HWYLA.tooltip.cagedmobs.cage.progress").mergeStyle(TextFormatting.GRAY).getString() +
        ...
        if(tile.hasEntity()){
            ...
            if(representation != null){
                tooltip.add(new StringTextComponent(
                        ...
                        new TranslationTextComponent(representation.getTranslationKey()).mergeStyle(TextFormatting.GRAY
                        ).getString()));
            }
        }
        // Add upgrades
        if(tile.isLightning()){
            tooltip.add(new TranslationTextComponent("item.cagedmobs.lightningupgrade").mergeStyle(TextFormatting.GRAY));
        }
        if(tile.isCooking()){
            tooltip.add(new TranslationTextComponent("item.cagedmobs.cookingupgrade").mergeStyle(TextFormatting.GRAY));
        }
        if (tile.isArrow()) {
            tooltip.add(new TranslationTextComponent("item.cagedmobs.arrowupgrade").mergeStyle(TextFormatting.GRAY));
        }
    }
